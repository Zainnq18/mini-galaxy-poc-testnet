...
async function getVoteTransactionHash(voting, voter, deployment, hasVoted) {
  ...
  const fallbackStart = Math.max(0, latestBlock - REGISTRY_DISCOVERY_LOOKBACK);
  const fromBlock =
  ...
  const log = await findLatestMatchingLog({
...
async function loadLatestRegistryDeployment() {
  ...
  try {
    ...
    console.warn("Could not load latest deployment from registry:", error.shortMessage || error.message);
  }

  return null;
}

async function refreshDeploymentCatalog({ force = false } = {}) {
  ...
  if (!force && catalogReady && Date.now() - catalogLastRefreshAt < CATALOG_REFRESH_TTL_MS) {
    return sortDeployments(deploymentCatalog.values());
  }
...
async function resolveDeployment(id = null, { strict = true } = {}) {
  ...
  const deployment = deploymentCatalog.get(key) || null;
  if (!deployment && strict) throw new Error("Voting event not found.");
  return deployment;
}
...
async function getConfig(eventId = null) {
  ...
  const contracts = await getContracts(deployment, false);
...
async function start() {
  ...
  refreshDeploymentCatalog({ force: true }).catch((error) => {
    catalogWarning = error.shortMessage || error.message;
    console.warn("Initial event catalog load failed:", catalogWarning);
  });
}
