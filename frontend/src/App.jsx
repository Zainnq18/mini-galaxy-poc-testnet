...
function VotePage({ account, eligibility, config, onConnect, refresh, setOperation, walletProvider }) {
  ...
  if (!eligibility?.eligible) return <section className="panel empty-state"><div className="empty-icon">—</div><h1>{eligibility?.viewOnlyReason || "View-only"}</h1><button className="secondary-button" onClick={() => navigate("/")}>Portal</button></section>;
